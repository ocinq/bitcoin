...
void MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const Consensus::Params& consensusParams, CBlockIndex *pindex = NULL, list<QueuedBlock>::iterator **pit = NULL) {
    ...
    if (itInFlight != mapBlocksInFlight.end() && itInFlight->second.first == nodeid) {
        if (pit && !itInFlight->second.second->partialBlock) {
            // If we are calling MarkBlockAsInFlight from a CMPCTBLOCK recv,
            // we may just be trying to set the partialBlockObject
            itInFlight->second.second->partialBlock.reset(new PartiallyDownloadedBlock(&mempool));
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    else if (inv.type == MSG_CMPCT_BLOCK)
                    {
                        ...
                        // if the request violates the protocol (ie they requested a block here
                        // which we did not announce to them), we may not respond to a corresponding
                        // GETBLOCKTXN message.
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        //
        {
            ...
            if (!fRevertToInv && vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
            ...
            } else if (state.fPreferHeaders && !vHeaders.empty()) {
                if (vHeaders.size() > 1) {
